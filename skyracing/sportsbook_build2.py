...
model_name="betfair_sportbook_uk_build2"
...
def place_bet(player,bet_amt="0.13"):
  ...
  return [max_val,min_val,placed_bet_id]
...
def model(dat):
  ...
  for t in sorted(highs,key=lambda x:x[0]['odds'])[::-1]:
    if t[0]['odds']>=1.1 and t[0]['odds']<=1.8:
      model_1=t[1]
      [max_val,min_val,bet_id]=place_bet(t[0],str(stake))
      break
  return [model_1,max_val,min_val,stake,bet_id]
