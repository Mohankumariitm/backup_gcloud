...
def slave_worker(cur_race_time):
  global loop_speed_thresh,horse_race_id,matchbook_session,betfair_session,list1,thresh,back_stake,back_odds,lay_stake,lay_odds
  win_marketid=list1[cur_race_time]
  start_time=int(time.time())
  flag=1
  while (int(time.time())-start_time)<900 and flag:
    try:
      r=requests.get("https://www.betfair.com/www/sports/exchange/readonly/v1/bymarket?_ak="+str(ak)+"&currencyCode=USD&locale=en&marketIds="+str(win_marketid)+"&rollupLimit=4&rollupModel=STAKE&types=MARKET_STATE")
      if r.json()['eventTypes'][0]['eventNodes'][0]['marketNodes'][0]['state']['inplay']:
        ...
        while (int(time.time())-race_start_time)<600 and flag:
          try:
            ...
            betfair_horse_map={c['selectionId']:int(c['description']['metadata']['CLOTH_NUMBER']) for c in r.json()['eventTypes'][0]['eventNodes'][0]['marketNodes'][0]['runners'] if 'description' in c and 'metadata' in c['description'] and 'CLOTH_NUMBER' in c['description']['metadata']}
            if betfair_horse_map:
              ...
              while (int(time.time())-race_start_time)<600 and flag:
                try:
                  ...
                  cur_hr=sorted([[t1['exchange']['availableToBack'][0]['price'],t1['selectionId']] for t1 in r.json()['eventTypes'][0]['eventNodes'][0]['marketNodes'][0]['runners'] if "exchange" in t1 and "availableToBack" in t1['exchange']])
                  if cur_hr:
                    if cur_hr[0][0]<=thresh:
                      if cur_hr[0][1] in betfair_horse_map:
                        pred=betfair_horse_map[cur_hr[0][1]]
                        if pred in matchbook_horse_map:
                          offers=[{"runner-id":matchbook_horse_map[pred]['runnerId'],"side":"back","odds":back_odds,"stake":back_stake}]+[{"runner-id":matchbook_horse_map[c]['runnerId'],"side":"lay","odds":lay_odds,"stake":lay_stake} for c in matchbook_horse_map if c!=pred]
                          ...
                          log("bet_placed","success",str(cur_hr))
                        else:
                          junk=1
                        flag=0
            ...
            log("slave_worker_loop2","Fail",str(traceback.format_exc()))
            time.sleep(1)#
      time.sleep(1)
    except:
...
while True:
  ...
  for t in sorted(list1):
    ...
    if cur_ts>=t-180 and cur_ts<=t+120 and t not in placed:
      placed.append(t)
      thread.start_new_thread(slave_worker,(t,))
