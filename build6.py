...
try:
 junk=requests.put("http://localhost:9200/uk_hr_all_build6/",json={"mappings": {"_doc": {"properties": {"timestamp": {"type": "date" }}}}})
except:
 junk=1
...
def oddschecker_list():
 r=requests.get("https://www.oddschecker.com/horse-racing")
 soup=BeautifulSoup(r.text,"html.parser")
 out={}
 for t in soup.find_all(class_="race-time"):
  if t.get("data-time"):
   race_time=int(time.mktime(dateutil.parser.parse(t.get("data-time").replace(" ","T")+".000Z").timetuple()))-3600
   if race_time not in out:
    out[race_time]="https://www.oddschecker.com"+t.get("href")
 return out
...
def racinguk_dat(cur_url):
 ...
 for t in soup.find_all(class_="racecard__runner--content"):
  try:
   ...
   fin_out.append({"odds":odds,"horse_number":hr_num,"horse_name":hr_name,"pacemap":pacemap,"tf_rating":tf_rating,"experience":1 if hr_num in out else 0})
  except:
   junk=1
 return fin_out
...
def update_res():
 r=requests.get("http://localhost:9200/uk_hr_all_build6/_doc/_search",json={"from" : 0, "size" : 10000,"query": {"bool": {"must_not": {"exists": {"field": "result_123"}}}}})
 for t in r.json()['hits']['hits']:
  ...
  if "race_details_dat" in tmp:
   ...
   junk=requests.post("http://localhost:9200/uk_hr_all_build6/_doc/"+str(t['_id']),json=tmp)
  ...
  for t in sorted(common_list):
   ...
   if cur_ts>=t-180 and cur_ts<=t+120 and t not in total_dat:
    try:
      ...
      r=requests.post("http://localhost:9200/uk_hr_all_build6/_doc/"+str(final_dat['timestamp']),json=final_dat)
      print r.text
