...
demo=False
min_stake=0.15
model_name="exchange_martingale_thresh_build_1"
...
while True:
  ...
  try:
        ...
        if abs(cur_ts-t)<=600 and t not in total_dat:
          ...
          if race_status in [3,4]:
            ...
            for hr in sorted(final['horse_data'],key=lambda x:x['odds']):
              if hr['odds']>=1.0 and hr['odds']<=6.0:
                model_pred=hr
                break
            if model_pred and model_pred['odds']>=3.9:
              r2=requests.get("http://localhost:9200/"+str(model_name)+"/_doc/_search",json={"from" : 0, "size" : 10000,"query": {"bool": {"must_not": {"exists": {"field": "result_123"}}}}})
              stake=round(min_stake,2)
              if r2.json()['hits']['hits']:
                hr=sorted(r2.json()['hits']['hits'],key=lambda x:x['_id'],reverse=True)[0]
                if hr['_source']["result_flag"]:
                  ...
                  stake=round(float(hr['_source']['model_pred_odds'])*hr['_source']['model_pred_stake'],2)
              if stake<=5.0 and not demo:
                matchbook_login()
    ...
    log(model_name+"_main_loop","Fail",str(traceback.format_exc()))
    time.sleep(2)
