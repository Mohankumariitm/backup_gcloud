...
race_list=get_list()
exchange_list=exchange_list_uk()
...
while True:
  ...
  for t in sorted(race_list):
    ...
    if (cur_ts-t)>=-600 and (cur_ts-t)<=600 and t not in total_dat:
      race_status=get_status(exchange_list[t]['race_id'])
      print race_status
      if race_status==3:
        ids={c.split("=")[0]:c.split("=")[1] for c in race_list[t].split("?")[1].split("&")}
        print t
