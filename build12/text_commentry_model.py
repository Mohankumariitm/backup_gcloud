  ...
  for t1 in soup1.find(id="ddMeetings").find_all("option"):
    if int(t1.get("value")):
      ...
      for t2 in soup2.find(id="pnlRaces").find_all("option"):
        if int(t2.get("value")):
          race_time=int(time.mktime(dateutil.parser.parse(datetime.date.today().strftime("%Y-%m-%d")+"T"+t2.text.strip()+".000Z").timetuple()))
      ...
      while (int(time.time())-start_time)<900 and flag:
        try:
          ...
          if cur<0:
            ...
            flag=0
            pred=sorted([[odds_map[c],c] for c in cur if odds_map[c]<=30.0])
            if pred and odds_map[pred[-1][1]]>=(sorted([c['midPrice'] for c in tmp['betfair_win_dat']])+[0.0,0.0,0.0])[3]:
              pred=pred[-1][1]
...
try:
  ...
  matchbook_login()
  log("init","Success",str(traceback.format_exc()))
except:
