...
def model_1(tmp):
  ...
  horses=[c for c in tmp['1_racinguk'] if c['horse_number'] in odds_map and not c['rp_tips'] and c['tf_stars']<=3]
  hr=sorted(horses,key=lambda x:odds_map[x['horse_number']])
  if hr and tmp['1_racinguk']:
    filter1=sorted([c['rp_tips'] for c in tmp['1_racinguk']])[-1] in range(2,6)
    filter2=odds_map[hr[0]['horse_number']]<=model_1_max_thresh
...
while True:
  ...
  try:
    ...
    log("loop","Fail",str(traceback.format_exc()))
