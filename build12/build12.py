...
def racinguk_list():
  ...
  for t in soup.find_all(class_="button button--link button--primary"):
    race_time=int(time.mktime(dateutil.parser.parse(datetime.date.today().strftime("%Y-%m-%d")+"T"+t.text.strip()+".000Z").timetuple()))
...
def racingpost_list():
  ...
  for t in soup.find_all(class_="RC-meetingItem"):
    try:
      if t.find(class_="RC-meetingItem__time").text.strip().split(":")[0] in ["12","11"]:
        race_time=int(time.mktime(dateutil.parser.parse(datetime.date.today().strftime("%Y-%m-%d")+"T"+t.find(class_="RC-meetingItem__time").text.strip()+".000Z").timetuple()))
        race_url="https://www.racingpost.com"+t.find("a").get("href")
        if race_time not in out:
          out[race_time]=race_url 
      else:
        race_time=int(time.mktime(dateutil.parser.parse(datetime.date.today().strftime("%Y-%m-%d")+"T"+t.find(class_="RC-meetingItem__time").text.strip()+".000Z").timetuple()))+43200
        race_url="https://www.racingpost.com"+t.find("a").get("href")
        if race_time not in out:
          out[race_time]=race_url 
    except:
...
def racingpost_tips(race_url):
  ...
  for t in soup.find_all(class_="RC-runnerRow"):
    try:
      horse_number=int(t.find(class_="RC-runnerNumber__no").text.strip())
      try:
        out[horse_number]=int(t.find(class_="RC-runnerStats__tips").text.split(" ")[0].strip())
      except:
        junk=1
    except:
      junk=1
...
def ladbrokes365_list():
  ...
  for t in soup.find_all(class_="rac-cards"):
    try:
      race_time=int(time.mktime(dateutil.parser.parse(datetime.date.today().strftime("%Y-%m-%d")+"T"+t.find(class_="ixt").text.strip()+".000Z").timetuple()))
...
def ladbrokes365_dat(cur_url):
  r=requests.get(cur_url+"/ratings")
  soup=BeautifulSoup(r.text,"html.parser")
  out1={}
  for t in soup.find_all("tr")[1:]:
    hr_num=int(t.find_all("td")[0].text.strip())
    rating=float(t.find_all("td")[2].find("span").text.strip())
    out1[hr_num]=rating
  return out1

def ladbrokes365_verdict(cur_url):
  r=requests.get(cur_url)
  soup=BeautifulSoup(r.text,"html.parser")
  try:
    horse_name=soup.find(class_="ruled").find(class_="highlight3").text.strip()
    r=requests.get(cur_url+"/ratings")
    soup=BeautifulSoup(r.text,"html.parser")
    out1={}
    for t in soup.find(id="racecard-tabs").find_all("tr")[1:]:
      hr_num=int(t.find_all("td")[0].text.strip())
      hr_name=str(t.find_all("td")[1].text.strip()).lower()
      out1[hr_name]=hr_num
    try:
      return out1[sorted([[NGram.compare(horse_name.lower(),c.lower()),c] for c in out1 if NGram.compare(horse_name.lower(),c.lower())>=0.25],reverse=True)[0][1]]
    except:
      return 0
  except:
    return 0
...
try:
  ...
  list5=ladbrokes365_list()
  common_list=[c for c in list1 if c in list2 and c in list3 and c in list4 and c in list5]
  ...
  log(model_name+"_init","Success","Total Races:"+str(len(common_list)))  
except:
...
while True:
  ...
  try:
    ...
    log(model_name+"_result_update","Fail",str(traceback.format_exc()))
  for t in sorted(common_list):
    ...
    if cur_ts>=t-180 and cur_ts<=t+120 and t not in total_dat:
      try:
        ...
        dm_dat=ladbrokes365_dat(list5[t])
        rp_tips=racingpost_tips(list4[t])
        verdict=ladbrokes365_verdict(list5[t])
        for t2 in final_dat['1_racinguk']:
          if t2['horse_number']==verdict:
            t2['365dm_verdict']=1
          else:
            t2['365dm_verdict']=0
          ...
          t2['365_rating']=dm_dat[t2['horse_number']] if t2['horse_number'] in dm_dat else 0
          t2['rp_tips']=rp_tips[t2['horse_number']] if t2['horse_number'] in rp_tips else 0
        ...
        total_dat[t]="added"
        log(model_name+"_new_race","Success",list4[t])
      except:
