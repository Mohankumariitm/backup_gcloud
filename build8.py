...
try:
  junk=requests.put("http://localhost:9200/uk_hr_all_build8/",json={"mappings": {"_doc": {"properties": {"timestamp": {"type": "date" }}}}})
except:
  junk=1
...
def update_res():
  r=requests.get("http://localhost:9200/uk_hr_all_build8/_doc/_search",json={"from" : 0, "size" : 10000,"query": {"bool": {"must_not": {"exists": {"field": "result_123"}}}}})
  for t in r.json()['hits']['hits']:
    ...
    if "race_details_dat" in tmp:
      ...
      junk=requests.post("http://localhost:9200/uk_hr_all_build8/_doc/"+str(t['_id']),json=tmp)
  ...
  for t in sorted(common_list):
    ...
    if cur_ts>=t-180 and cur_ts<=t+120 and t not in total_dat:
      try:
        ...
        r=requests.post("http://localhost:9200/uk_hr_all_build8/_doc/"+str(final_dat['timestamp']),json=final_dat)
        print r.text
